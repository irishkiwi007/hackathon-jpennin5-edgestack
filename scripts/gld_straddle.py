...
out, tok = [], None
while True:
    a = ['data', 'bars', '--symbol', 'GLD', '--timeframe', '1Day', '--start', '2023-10-01',
...
def occ(exp, cp, k):
    return f'GLD{exp:%y%m%d}{cp}{int(round(k*1000)):08d}'


# entries: every Monday with a session ~HOLD later; expiry = that session
cycles = []
for i, d in enumerate(dts):
    if d.weekday() != 0 or i + HOLD + 6 >= N or i < 21:
        continue
    # nearest FRIDAY session at least HOLD sessions out (Fridays are always SPY expiries)
    j = None
...
def straddle(i, j):
    ...
    for dk in (0, 1, -1, 2, -2, 3, -3):
        kk = round(S0) + dk
        if PX.get((occ(exp, 'C', kk), ed)) is not None and            PX.get((occ(exp, 'P', kk), ed)) is not None:
            k = kk
            break
    if k is None:
        return None
    cp_ = PX[(occ(exp, 'C', k), ed)]
    pp_ = PX[(occ(exp, 'P', k), ed)]
    prem = cp_ + pp_
    payoff = abs(S1 - k)
    ...
    return dict(prem=prem, k=k, S0=S0, S1=S1, payoff=payoff,
                ...
                iv_proxy=prem / S0 / math.sqrt(max((exp - dts_i).days,1) / 365) * 0.8)
...
def summarise(g, label):
    ...
    sm = sum(ss) / n
    sd = math.sqrt(sum((x - sm) ** 2 for x in ss) / (n - 1)) if n > 1 else 0
    t = sm / (sd / math.sqrt(n)) if sd else 0
    sw = sum(1 for x in ss if x > 0) / n
    lm = -sm
    lw = sum(1 for x in g if x['long_pnl'] > 0) / n
    return (f'{label:<24} {n:>4} {sum(x["rv"] for x in g)/n*100:>7.1f}% '
            f'{sum(x["ivrv"] for x in g)/n:>6.2f} '
            f'{lm:>9.0f} {lw*100:>6.1f}% {sm:>10.0f} {sw*100:>6.1f}% {sd:>8.0f} {t:>6.2f} '
            f'{"SIG" if abs(t)>1.96 else "-":>4}')


print('\n' + '=' * 104)
print('ATM STRADDLE OUTCOMES BY TRAILING-VOL TERCILE  (real prices, $5/leg slippage)')
print('=' * 104)
print(f'{"bucket":<24} {"n":>4} {"mean RV":>8} {"IV/RV":>6} '
      f'{"LONG$":>9} {"win%":>7} {"SHORT$":>10} {"win%":>7} {"sd":>8} {"t":>6} {"":>4}')
